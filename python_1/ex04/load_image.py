...
def print_2D_transpose(array):
    """
    Print a formatted output of the 3 first and last values
    of the first and last row of the image array.
    """
    print(
        f"[[ {array[0][0]} {array[0][1]} {array[0][2]}"
        " ... "
        f"{array[0][-3]} {array[0][-2]} {array[0][-1]}]"
        "\n   ...   \n"
        f" [ {array[-1][0]} {array[-1][1]} {array[-1][2]}"
        " ... "
        f"{array[-1][-3]} {array[-1][-2]} {array[-1][-1]}]]"
    )
...
def print_array(array):
    ...
    try:
        ...
        if array.ndim == 3:  # Use .ndim for clarity
...
def ft_load_as_gray(path: str) -> np.array:
    """
    Assert that the path is valid
    Print the dimension of the image
    Return the RGB value of every pixel
